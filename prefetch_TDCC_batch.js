...
function log(message) {
    // stderr，不污染 PHP shell_exec() JSON
...
async function initializePage(browser, targetDate) {
    ...
    const dateExists = await page.evaluate(
        targetDate => {
            ...
            const select =
                document.querySelector('#scaDate');
...
async function isPageHealthy(page) {
    ...
    try {
        ...
        const scaDateExists = await page.$(
            '#scaDate'
        );

        const stockNoExists = await page.$(
            '#StockNo'
        );
            ...
            if (!success) {
                ...
                if (
                    ...
                    try {
                        ...
                        log(
                            `🔴 Browser 重啟失敗: ${restartError.message}`
                        );
                    }
                }
            }
